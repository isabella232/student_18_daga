...
func testServerProtocolsOnClientRequests(context daga.AuthenticationContext, servers []daga.Server) func(commits []kyber.Point)daga.Challenge {
	...
	sendCommitsReceiveChallenge := func(proverCommitments []kyber.Point)daga.Challenge {
		...
		sendCom, err := daga_login.NetEncodeCommitment(comlead)
		if err != nil {
		...
		for num, server := range servers {
			...
			sendCom, e := daga_login.NetEncodeCommitment(com)
			if e != nil {
		...
		sendChall, err := daga_login.NetEncodeChallengeCheck(challenge)
		if err != nil {
		...
		for shift := 1; shift <= len(Y); shift++ {
			...
			sendservChall, e := daga_login.NetEncodeChallengeCheck(serverChallenge)
			if e != nil {
...
func TestScenario(t *testing.T) {
	...
	sendclientMsg, err := daga_login.NetEncodeAuthenticationMessage(msg)
	if err != nil {
	...
	//Network transfer
	var rcvclientMsg daga_login.NetAuthenticationMessage
	...
	for shift := range servers {
		...
		sendservMsg, e := daga_login.NetEncodeServerMessage(suite, msgServ)
		if e != nil {
