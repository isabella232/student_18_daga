...
func (p *Protocol) LeaderSetup(req *daga_login.CreateContext) {
	...
	if req == nil || req.DagaNodes == nil || req.ServiceID == daga_login.ServiceID(uuid.Nil) || len(req.Signature) == 0 || len(req.SubscribersKeys) == 0 {
...
func (p *Protocol) Start() (err error) {
	...
	p.result = make(chan daga_login.Context)
...
func (p *Protocol) WaitForResult() (daga_login.Context, daga.Server, error) {
	if p.result == nil {
	...
	case <-time.After(Timeout):
		return daga_login.Context{}, nil, fmt.Errorf("%s didn't finish in time", Name)
	}
}
...
func (p *Protocol) HandleSign(msg StructSign) (err error) {
	...
	if !daga_login.ContainsSameElems(p.originalRequest.SubscribersKeys, msg.Context.G.X) {
...
func (p *Protocol) HandleSignReply(msg []StructSignReply) (err error) {
	...
	finalContext, err := daga_login.NewContext(*p.context, p.Roster(), p.context.ServiceID, p.context.Signatures)
	if err != nil {
		return fmt.Errorf("%s: failed to handle SignReply: %s", Name, err)
	}
